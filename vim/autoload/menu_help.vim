...
function! menu_help#PlugEvent(event)
	exec "normal \<plug>(" . a:event . ")"
endfunc

function! menu_help#FormatJson()
	exec "%!python -m json.tool"
endfunc

function! menu_help#Fscope(scope)
	exec "GscopeFind ". a:scope . " " . fnameescape(expand('<cword>'))
endfunc

function! menu_help#Gscope(what)
...
function! menu_help#HelpList(name, content)
	let content = []
	let index = 0
	for item in a:content
		...
		let cmd = 'call menu_help#WinHelp("' . item[1] . '")'
...
function! menu_help#FeedPlug(text)
